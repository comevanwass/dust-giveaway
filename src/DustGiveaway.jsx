...
const STORAGE_KEY = "dust-fest-participants";
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dg-admin-trigger {
    ...
    color: #2C2722; cursor: pointer; background: none; border: none;
  ...
  .dg-success-mark {
    font-size: 14px; letter-spacing: 0.5em; color: #ffa800;
  ...
  .dg-success-title {
    ...
    color: #ffa800; letter-spacing: 0.04em; line-height: 0.9;
    margin-bottom: 20px;
  }
  ...
  .dg-login-page {
    min-height: 100vh; display: flex; flex-direction: column;
    align-items: center; justify-content: center;
    padding: 40px 24px; animation: fadeUp 0.4s ease both;
  }

  .dg-login-title {
    font-family: 'Bebas Neue', sans-serif;
    font-size: 28px; letter-spacing: 0.14em; color: #ffffff; margin-bottom: 28px;
  }
  ...
  .dg-pw-input {
    ...
    padding: 12px 16px; outline: none; margin-bottom: 14px;
    transition: border-color 0.2s;
  }
  .dg-pw-input:focus { border-color: #C9A97F; }

  .dg-login-btn {
    width: 100%; max-width: 280px; background: transparent;
    border: 1px solid #ffa800; color: #ffa800;
    font-family: 'Bebas Neue', sans-serif; font-size: 15px;
    letter-spacing: 0.2em; padding: 12px; cursor: pointer;
    transition: background 0.18s, color 0.18s;
  }
  .dg-login-btn:hover { background: #ffa800; color: #0D0B09; }

  .dg-login-err {
    font-size: 10px; color: #A84040; margin-top: 14px; letter-spacing: 0.12em;
  }
  ...
  .dg-back-btn:hover { color: #EDE8E1; }

  /* ───── ADMIN DASHBOARD ───── */
  .dg-admin-page {
    min-height: 100vh; padding: 44px 36px 80px;
  ...
  .dg-admin-title {
    font-family: 'Bebas Neue', sans-serif;
    font-size: 34px; letter-spacing: 0.08em; color: #ffa800;
  }
  ...
  .dg-pt-head {
    ...
    color: #3D3630; gap: 16px;
  }

  .dg-pt-row {
    display: grid; grid-template-columns: 1fr 1fr 1fr 2fr;
    padding: 13px 0; border-bottom: 1px solid #1A1714;
    gap: 16px; align-items: start; transition: background 0.2s;
  }
  .dg-pt-row.is-winner { background: rgba(201,169,127,0.06); }
  ...
  .dg-pt-badge {
    ...
    color: #ffa800; background: rgba(255,168,0,0.12);
    padding: 3px 8px; display: inline-block; margin-top: 2px;
  }

  .dg-empty {
    text-align: center; padding: 56px 0;
    color: #2C2722; font-size: 12px; letter-spacing: 0.2em; text-transform: uppercase;
  }
  ...
  .dg-winners-grid {
    ...
    gap: 14px; margin: 0 0 32px; text-align: center;
  }
  ...
  .dg-winner-card {
    ...
    padding: 20px 16px;
    opacity: 0;
    animation: winIn 0.45s ease forwards;
  }
  ...
  .dg-w-num {
    ...
    color: #ffa800; margin-bottom: 8px;
  }
  .dg-w-name {
    font-family: 'Bebas Neue', sans-serif;
    font-size: 26px; letter-spacing: 0.06em; color: #ffffff;
  }
  ...
  .dg-draw-btn {
    ...
    font-size: 20px; letter-spacing: 0.16em; cursor: pointer;
  ...
  .dg-draw-hint {
    font-size: 10px; color: #3D3630; margin-top: 12px; letter-spacing: 0.1em;
  }
  ...
  .dg-ghost-btn:hover { border-color: #ffa800; color: #ffa800; }
  ...
  .dg-export-btn:hover { border-color: #ffa800; color: #ffa800; }
`;
...
export default function DustGiveaway() {
  ...
  async function loadParticipants() {
    try {
      ...
      if (r && r.value) setParticipants(JSON.parse(r.value));
    } catch {}
  }

  async function persistParticipants(list) {
  ...
  async function handleSubmit() {
    ...
    const updated = [...participants, entry];
  ...
  const drawWinners = useCallback(() => {
    ...
    const allNames = participants.map(p => `${p.firstName} ${p.lastName}`);
    ...
    function revealOne(idx) {
      ...
      const target = `${chosen[idx].firstName} ${chosen[idx].lastName}`;
      let count = 0;
  ...
  function exportCSV() {
    const rows = [
      ...
      ...participants.map(p => [p.firstName, p.lastName, p.phone, `"${p.answer.replace(/"/g, '""')}"`, p.at]),
    ];
      ...
      <div className="dg-form-page">
        ...
        <div className="dg-card">
          ...
          <div className={`dg-field${errors.phone ? " err" : ""}`}>
            ...
            <input type="tel" placeholder="+32 470 000 000"
              value={form.phone}
          ...
          <div className={`dg-field${errors.answer ? " err" : ""}`}>
            ...
            <textarea rows={3} placeholder="Tell us..."
              value={form.answer}
    ...
    <div className="dg-root">
      ...
      <div className="dg-admin-page">
        ...
        {participants.length === 0 ? (
          <div className="dg-empty">No participants yet</div>
        ) : (
          <div className="dg-ptable">
            <div className="dg-pt-head">
              <span>First</span>
              <span>Last</span>
              <span>Phone</span>
              <span>Answer</span>
            ...
            {participants.map(p => (
              <div key={p.id} className={`dg-pt-row${winnerIds.has(p.id) ? " is-winner" : ""}`}>
                <div>
                  <div className="dg-pt-name">{p.firstName}</div>
                  {winnerIds.has(p.id) && <div className="dg-pt-badge">Winner</div>}
                </div>
                <div className="dg-pt-name">{p.lastName}</div>
        ...
        <div className="dg-draw-section">
          ...
          {winners.length > 0 && (
            <div className="dg-winners-grid">
              {winners.map((w, i) => (
                <div key={w.id} className="dg-winner-card"
                  style={{ animationDelay: `${i * 0.08}s` }}>
                  <div className="dg-w-num">Winner {i + 1}</div>
                  <div className="dg-w-name">{w.firstName} {w.lastName}</div>
          ...
          {!drawing && (
            <div>
              <button
                className="dg-draw-btn"
                onClick={drawWinners}
                disabled={participants.length < WINNER_COUNT}
              >
                {winners.length > 0 ? "REDRAW WINNERS" : "DRAW 4 WINNERS"}
              </button>
              {participants.length < WINNER_COUNT && (
                <div className="dg-draw-hint">
                  Need at least {WINNER_COUNT} participants to draw
                </div>
              )}
            </div>
          )}
          ...
          {participants.length > 0 && !drawing && (
            <div className="dg-actions">
              <button className="dg-export-btn" onClick={exportCSV}>
                Export CSV
              </button>
              <button className="dg-ghost-btn" onClick={clearAll}>
